...
import { TOOLS, createMcpServer, } from "./server-setup.js";
import { handleAciClassify, handleAciFinancialGate, handleAciComplianceCheck, handleAciRouteDomain, handleAciParallelReview, handleAciSynthesizeReviews, handleAciLogDecision, handleAciRecallPrecedent, handleAciRecordOutcome, handleAciBlastRadius, handleAciRollback, handleAciGovernanceHealth, } from "./handlers.js";
// ---------------------------------------------------------------------------
// Shared mock API client factory
// ---------------------------------------------------------------------------
function createMockApiClient(responseData = {}) {
...
describe("ACI Tool Input Schemas", () => {
    describe("aci_classify_decision", () => {
        ...
        it("should define environment enum", () => {
            const envProp = tool.inputSchema.properties
                .environment;
        ...
        it("should define context as an object with domain enum", () => {
            const ctxProp = tool.inputSchema.properties
                .context;
    ...
    describe("aci_financial_gate", () => {
        ...
        it("should define duration_hours as number", () => {
            const prop = tool.inputSchema.properties
                .duration_hours;
    ...
    describe("aci_recall_precedent", () => {
        ...
        it("should define top_k with a default of 5", () => {
            const prop = tool.inputSchema.properties.top_k;
...
describe("handleAciClassify", () => {
    it("should call the correct API endpoint with params", async () => {
        ...
        const result = await handleAciClassify({ action: "list all S3 buckets", environment: "development" }, client);
    ...
    it("should throw when action is missing", async () => {
        ...
        await expect(handleAciClassify({}, client)).rejects.toThrow("Required parameter 'action' must be a non-empty string");
    });
    it("should throw when action is not a string", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciClassify({ action: 123 }, client)).rejects.toThrow("Required parameter 'action' must be a non-empty string");
    });
    it("should include domain scores in output when present", async () => {
        ...
        const result = await handleAciClassify({ action: "modify IAM policy in production", environment: "production" }, client);
    ...
    it("should handle BLOCK classification", async () => {
        ...
        const result = await handleAciClassify({ action: "rm -rf /prod/database" }, client);
    ...
    it("should pass org_config when provided", async () => {
        ...
        expect(callApi).toHaveBeenCalledWith("/aci/classify", "POST", expect.objectContaining({
            org_config: { risk_tolerance: "aggressive", require_human_approval_above: 0.9 },
        }));
    });
    it("should handle API failure gracefully", async () => {
        const { client } = createMockApiClient();
        client.callApi.mockRejectedValue(new Error("API timeout"));
        await expect(handleAciClassify({ action: "test action" }, client)).rejects.toThrow("API timeout");
    });
});
// ===========================================================================
...
describe("handleAciFinancialGate", () => {
    it("should call the correct API endpoint", async () => {
        ...
        const result = await handleAciFinancialGate({ action: "echo hello world" }, client);
    ...
    it("should throw when action is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciFinancialGate({}, client)).rejects.toThrow("Required parameter 'action'");
    });
    it("should display cost estimates for expensive actions", async () => {
        ...
        const result = await handleAciFinancialGate({ action: "Launch 1x m5.xlarge EC2 instance", duration_hours: 730 }, client);
        expect(result).toContain("ESCALATE");
...
describe("handleAciComplianceCheck", () => {
    it("should return PASS for compliant actions", async () => {
        ...
        const result = await handleAciComplianceCheck({ action: "read-only query on test database" }, client);
    ...
    it("should throw when action is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciComplianceCheck({}, client)).rejects.toThrow("Required parameter 'action'");
    });
    it("should handle WARN result with warnings", async () => {
        ...
        const result = await handleAciComplianceCheck({ action: "deploy new service to staging" }, client);
...
describe("handleAciRouteDomain", () => {
    it("should route security tasks to security domain", async () => {
        ...
        const result = await handleAciRouteDomain({ task_description: "Review IAM policy changes for production account" }, client);
    ...
    it("should route financial tasks to financial domain", async () => {
        ...
        const result = await handleAciRouteDomain({ task_description: "Approve purchase order for $50,000 cloud spend" }, client);
        expect(result).toContain("financial");
        expect(result).toContain("cfo-agent");
    });
    it("should throw when task_description is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciRouteDomain({}, client)).rejects.toThrow("Required parameter 'task_description'");
    });
});
// ===========================================================================
...
describe("handleAciParallelReview", () => {
    ...
    it("should throw when action is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciParallelReview({ reviewers: ["security"] }, client)).rejects.toThrow("Required parameter 'action'");
    });
    it("should throw when reviewers is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciParallelReview({ action: "some action" }, client)).rejects.toThrow("Required parameter 'reviewers' must be an array");
    });
    it("should throw when reviewers is not an array", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciParallelReview({ action: "some action", reviewers: "security" }, client)).rejects.toThrow("Required parameter 'reviewers' must be an array");
    });
});
// ===========================================================================
...
describe("handleAciSynthesizeReviews", () => {
    ...
    it("should throw when review_session_id is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciSynthesizeReviews({ reviews: [] }, client)).rejects.toThrow("Required parameter 'review_session_id'");
    });
    it("should throw when reviews is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciSynthesizeReviews({ review_session_id: "S1" }, client)).rejects.toThrow("Required parameter 'reviews' must be an array");
    });
});
// ===========================================================================
...
describe("handleAciLogDecision", () => {
    ...
    it("should throw when action is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciLogDecision({ classification: "APPROVE", reasoning: "ok" }, client)).rejects.toThrow("Required parameter 'action'");
    });
    it("should throw when classification is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciLogDecision({ action: "test", reasoning: "ok" }, client)).rejects.toThrow("Required parameter 'classification'");
    });
    it("should throw when reasoning is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciLogDecision({ action: "test", classification: "APPROVE" }, client)).rejects.toThrow("Required parameter 'reasoning'");
    });
});
// ===========================================================================
...
describe("handleAciRecallPrecedent", () => {
    it("should return matching precedents", async () => {
        ...
        const result = await handleAciRecallPrecedent({ query: "deploy application to production" }, client);
    ...
    it("should return empty message for no matches", async () => {
        ...
        const result = await handleAciRecallPrecedent({ query: "something completely unique and unmatched" }, client);
    ...
    it("should throw when query is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciRecallPrecedent({}, client)).rejects.toThrow("Required parameter 'query'");
    });
...
describe("handleAciRecordOutcome", () => {
    it("should record a positive outcome successfully", async () => {
        ...
        const result = await handleAciRecordOutcome({ decision_id: "D-100", outcome: "SUCCESS", outcome_details: "Deployed cleanly" }, client);
    ...
    it("should record a negative outcome and adjust confidence", async () => {
        ...
        const result = await handleAciRecordOutcome({ decision_id: "D-200", outcome: "FAILURE" }, client);
        expect(result).toContain("D-200");
        expect(result).toContain("-0.10");
    });
    it("should throw when decision_id is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciRecordOutcome({ outcome: "SUCCESS" }, client)).rejects.toThrow("Required parameter 'decision_id'");
    });
    it("should throw when outcome is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciRecordOutcome({ decision_id: "D-100" }, client)).rejects.toThrow("Required parameter 'outcome'");
    });
});
// ===========================================================================
...
describe("handleAciBlastRadius", () => {
    it("should return LOW scope for single-file change", async () => {
        ...
        const result = await handleAciBlastRadius({ action: "Update README.md in dev branch" }, client);
    ...
    it("should return HIGH scope for production deployment", async () => {
        ...
        const result = await handleAciBlastRadius({ action: "Deploy v3.0 to production", environment: "production" }, client);
    ...
    it("should return CRITICAL scope for database migration", async () => {
        ...
        const result = await handleAciBlastRadius({ action: "DROP TABLE users in production" }, client);
    ...
    it("should throw when action is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciBlastRadius({}, client)).rejects.toThrow("Required parameter 'action'");
    });
});
// ===========================================================================
...
describe("handleAciRollback", () => {
    it("should generate rollback steps for a deployment", async () => {
        ...
        const result = await handleAciRollback({ action: "Deploy v3.0 to production ECS cluster" }, client);
    ...
    it("should handle database rollback steps", async () => {
        ...
        const result = await handleAciRollback({ action: "Run database migration ALTER TABLE users ADD COLUMN" }, client);
    ...
    it("should throw when action is missing", async () => {
        const { client } = createMockApiClient();
        await expect(handleAciRollback({}, client)).rejects.toThrow("Required parameter 'action'");
    });
});
// ===========================================================================
...
describe("handleAciGovernanceHealth", () => {
    ...
    it("should pass custom time_range and domain filter", async () => {
        ...
        await handleAciGovernanceHealth({ time_range: "7d", domain: "security" }, client);
        expect(callApi).toHaveBeenCalledWith("/aci/health", "POST", {
            time_range: "7d",
            domain: "security",
        });
...
describe("Error Propagation", () => {
    ...
    for (const { name, fn, args } of handlers) {
        it(`${name} should propagate API errors`, async () => {
            const { client } = createMockApiClient();
            client.callApi.mockRejectedValue(new Error("Network failure: 503"));
