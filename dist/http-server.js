...
import { createApiClient, createMcpServer, SERVER_NAME, SERVER_VERSION, } from "./server-setup.js";
...
const app = express();
// -- CORS ------------------------------------------------------------------
const ALLOWED_ORIGINS = new Set((process.env.S2T_CORS_ORIGINS || "").split(",").filter(Boolean));
function isOriginAllowed(origin) {
    if (!origin)
        return true; // Allow same-origin / non-browser requests
    if (ALLOWED_ORIGINS.size === 0)
        return true; // No restriction if env not set (local dev)
...
app.use((req, res, next) => {
    ...
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, Accept, Mcp-Session-Id, Last-Event-ID, X-S2T-API-Key");
...
app.post("/mcp", async (req, res) => {
    ...
    if (sessionId && streamableTransports.has(sessionId)) {
        ...
        // Clean up on close
        transport.onclose = () => {
            const sid = Array.from(streamableTransports.entries()).find(([, t]) => t === transport)?.[0];
            if (sid) {
...
app.post("/messages", async (req, res) => {
    ...
    for (const transport of sseTransports.values()) {
        ...
        if (transport._sessionId === sessionId) {
            matched = transport;
            break;
        }
    }
