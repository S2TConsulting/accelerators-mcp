...
// Configuration
const API_BASE_URL = process.env.S2T_API_URL ||
...
if (!API_KEY) {
    ...
    console.error("Get your API key at: https://dev.s2tconsulting.com/ai-sales/purchase.html");
    process.exit(1);
}
// Tool definitions
export const TOOLS = [
    {
        name: "s2t_embed",
        description: "Generate vector embeddings for text using S2T's embedding pipeline. Supports chunking for long documents. Use for RAG, semantic search, and document indexing.",
        inputSchema: {
            type: "object",
            properties: {
                text: {
                    type: "string",
                    description: "The text to generate embeddings for (max 100,000 characters)",
                },
                model: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Generate production-ready SAM/CloudFormation templates from natural language descriptions. Creates Lambda functions, API Gateway, DynamoDB tables, S3 buckets with security best practices.",
        inputSchema: {
            type: "object",
            properties: {
                description: {
                    type: "string",
                    description: "Natural language description of the infrastructure needed (max 5000 chars)",
                },
                format: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Validate OAuth 2.0 configuration and detect common misconfigurations. Supports Google, Microsoft, GitHub, QuickBooks, and generic providers.",
        inputSchema: {
            type: "object",
            properties: {
                provider: {
                ...
                token_endpoint: {
                    type: "string",
                    description: "Custom token endpoint URL (optional, for generic provider)",
                },
                authorization_endpoint: {
                    type: "string",
                    description: "Custom authorization endpoint URL (optional, for generic provider)",
                },
            },
    ...
    },
    {
        ...
        description: "Validate IAM policies for security best practices. Detects overly permissive permissions, dangerous actions, and suggests least-privilege alternatives. Returns a security score (0-100) and detailed findings.",
        inputSchema: {
            type: "object",
            properties: {
                policy_document: {
                    type: ["string", "object"],
                    description: "The IAM policy document to validate (JSON string or object)",
                },
                resource_type: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Validate IAM users and roles for MFA compliance. Checks console access, programmatic access, root account, and policy conditions. Returns compliance score and remediation recommendations.",
        inputSchema: {
            type: "object",
            properties: {
                users: {
    ...
    },
    {
        ...
        description: "Generate optimal DynamoDB single-table designs from entity and access pattern descriptions. Returns key schema, GSI recommendations, entity mappings, and CloudFormation template.",
        inputSchema: {
            type: "object",
            properties: {
                entities: {
    ...
    },
    {
        ...
        description: "Analyze application error logs to identify patterns, root causes, and trends. Categorizes errors by type and severity, suggests remediation actions.",
        inputSchema: {
            type: "object",
            properties: {
                errors: {
    ...
    },
    {
        ...
        description: "Assess data lake architecture readiness for production. Evaluates storage, catalog, security, performance, and operations. Returns readiness score and recommendations.",
        inputSchema: {
            type: "object",
            properties: {
                storage: {
    ...
    {
        name: "s2t_catalog",
        description: "List all available S2T accelerators with their capabilities, pricing, and usage information.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        ...
        description: "Classify an action's risk level (LOW/MEDIUM/HIGH/CRITICAL) based on blast radius, reversibility, and environment. Use for AI agent safety guardrails and graduated autonomy.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        ...
        description: "Route a task to the optimal AI agent using semantic similarity matching against 54 specialized agent domains. Returns ranked candidates with confidence scores.",
        inputSchema: {
            type: "object",
            properties: {
                task_description: {
    ...
    },
    {
        ...
        description: "Predict upcoming system issues by analyzing budget trends, error rates, dependency health, and certificate expiry. Returns prioritized predictions with recommended actions.",
        inputSchema: {
            type: "object",
            properties: {
                system_state: {
    ...
    },
    {
        ...
        description: "Match an error against known patterns and suggest automated recovery steps. Includes historical success rates and confidence scores for each recovery strategy.",
        inputSchema: {
            type: "object",
            properties: {
                error_message: {
    ...
    },
    {
        ...
        description: "Configure resilience patterns (retry with exponential backoff, circuit breaker) for an operation. Returns execution metrics and circuit breaker state.",
        inputSchema: {
            type: "object",
            properties: {
                operation_id: {
    ...
    },
    {
        ...
        description: "Store, retrieve, search, or delete persistent agent memory with namespace isolation. Use for maintaining agent state across sessions.",
        inputSchema: {
            type: "object",
            properties: {
                operation: {
    ...
    },
    {
        ...
        description: "Submit a task to the SQS FIFO queue for asynchronous multi-agent execution. Returns task ID and queue position for tracking.",
        inputSchema: {
            type: "object",
            properties: {
                agent_id: {
    ...
    },
    {
        ...
        description: "Generate W3C Trace Context identifiers for distributed tracing across multi-agent workflows. Supports parent-child span relationships.",
        inputSchema: {
            type: "object",
            properties: {
                parent_traceparent: {
    ...
    },
    {
        ...
        description: "Acquire or release file-based mutex locks with stale lock detection. Prevents concurrent file access conflicts in multi-agent environments.",
        inputSchema: {
            type: "object",
            properties: {
                file_path: {
    ...
    },
    {
        ...
        description: "Validate CLI tool availability and API key health. Returns readiness status with degradation recommendations if tools are unavailable.",
        inputSchema: {
            type: "object",
            properties: {
                cli_tools: {
