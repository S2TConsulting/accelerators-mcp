...
import { handleEmbed, handleCloudFormation, handleOAuthValidate, handleIamPolicyValidate, handleMfaCompliance, handleDynamoDbDesign, handleErrorPatterns, handleDataLakeReadiness, handleCatalog, handleUsage, handleRiskClassify, handleTaskRouting, handlePredictIssues, handleAutoRecovery, handleResilienceExecute, handleAgentMemory, handleAgentTask, handleTraceContext, handleFileLock, handleCliReadiness, 
...
export const TOOLS = [
    ...
    // =========================================================================
    {
        ...
        description: "Generate vector embeddings for text using Amazon Bedrock Titan models. Supports automatic chunking for long documents. Use when building RAG pipelines, semantic search, document similarity, or knowledge base indexing. Returns embedding vectors with chunk metadata. No side effects -- read-only computation.",
        inputSchema: {
            type: "object",
            properties: {
                text: {
    ...
    },
    {
        ...
        description: "Analyze application error logs to identify recurring patterns, root causes, and trends. Use when debugging production issues, performing post-mortems, or monitoring error spikes. Returns categorized patterns with severity and remediation suggestions. No side effects -- read-only analysis.",
        inputSchema: {
            type: "object",
            properties: {
                errors: {
    ...
    },
    {
        ...
        description: "Assess data lake architecture readiness for production deployment. Evaluates five dimensions: S3 storage configuration, Glue catalog setup, security controls, query performance, and operational tooling. Use before promoting a data lake to production. Returns readiness score (0-100) and prioritized recommendations. No side effects -- read-only assessment.",
        inputSchema: {
            type: "object",
            properties: {
                storage: {
    ...
    },
    {
        ...
        description: "Validate CLI tool availability and API key health on the current system. Use before running agent workflows that depend on external CLIs (codex, claude, git). Returns readiness status per tool with degradation mode recommendations if tools are missing. No side effects -- read-only system check.",
        inputSchema: {
            type: "object",
            properties: {
                cli_tools: {
    ...
    // =========================================================================
    {
        ...
        description: "List all available S2T accelerators with their capabilities, pricing tiers, and current usage information. Use to discover available tools, check tier access, or review pricing. Returns the full accelerator catalog with metadata. No side effects -- read-only query.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    },
    {
        name: "s2t_usage",
        title: "Get API Usage",
        description: "Get your current API usage statistics and remaining quota for the billing period. Use to monitor consumption, check rate limits, or verify tier status. Returns requests made, tokens used, remaining quota, and current tier. No side effects -- read-only query.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    // =========================================================================
    {
        ...
        description: "Classify an action's risk level as LOW, MEDIUM, HIGH, or CRITICAL based on blast radius, reversibility, and target environment. Use for AI agent safety guardrails and graduated autonomy decisions. Returns risk classification with scoring breakdown. No side effects -- read-only classification.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        ...
        description: "Route a task to the optimal AI agent using semantic similarity matching against 54 specialized agent domains. Use when delegating work across a multi-agent system. Returns ranked agent candidates with confidence scores and capability summaries. No side effects -- read-only routing decision.",
        inputSchema: {
            type: "object",
            properties: {
                task_description: {
    ...
    },
    {
        ...
        description: "Predict upcoming system issues by analyzing budget trends, error rates, dependency health, and certificate expiry. Use for proactive operations monitoring. Returns prioritized predictions with severity, projected impact dates, and recommended preventive actions. No side effects -- read-only analysis.",
        inputSchema: {
            type: "object",
            properties: {
                system_state: {
    ...
    },
    {
        ...
        description: "Configure resilience patterns (retry with exponential backoff, circuit breaker) for an operation. Use when protecting critical operations against transient failures. Returns execution configuration with retry metrics and circuit breaker state. Records resilience configuration as a side effect.",
        inputSchema: {
            type: "object",
            properties: {
                operation_id: {
    ...
    },
    {
        ...
        description: "Store, retrieve, search, or delete persistent agent memory with namespace isolation. Use for maintaining agent state across sessions, caching intermediate results, or sharing context between agents. Side effects depend on operation: store/delete modify state; retrieve/search are read-only.",
        inputSchema: {
            type: "object",
            properties: {
                operation: {
    ...
    },
    {
        ...
        description: "Submit a task to the SQS FIFO queue for asynchronous multi-agent execution. Use when delegating work to background agents or building agent pipelines. Returns task ID and queue position for tracking. Side effect: enqueues a message to SQS.",
        inputSchema: {
            type: "object",
            properties: {
                agent_id: {
    ...
    // =========================================================================
    {
        ...
        description: "Generate W3C Trace Context identifiers (traceparent + tracestate) for distributed tracing across multi-agent workflows. Use when starting new traces or creating child spans within existing traces. Returns W3C-compliant trace headers. No side effects -- generates identifiers only.",
        inputSchema: {
            type: "object",
            properties: {
                parent_traceparent: {
    ...
    },
    {
        ...
        description: "Acquire, release, or check file-based mutex locks with stale lock detection. Use to prevent concurrent file access conflicts in multi-agent environments. Side effects: acquire creates a lock file; release removes it; check is read-only.",
        inputSchema: {
            type: "object",
            properties: {
                file_path: {
    ...
    },
    {
        ...
        description: "Estimate the financial impact of an action and determine if it passes budget gates. Use before any action with cost implications (provisioning resources, purchasing services, scaling infrastructure). Returns cost breakdown, budget impact analysis, and APPROVE/ESCALATE/BLOCK decision. Records audit entry as side effect.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        ...
        description: "Evaluate an action against compliance frameworks (SOC2, GDPR, HIPAA, PCI-DSS). Use before actions that handle sensitive data or affect auditable systems. Returns framework-specific violations, warnings, and remediation steps. No side effects beyond audit logging.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        ...
        description: "Route a governance task to the appropriate domain expert agent(s) with sensitivity tagging. Extends s2t_route_task_to_agent with governance awareness and parallel review recommendations. Use when a governance decision needs domain-specific expertise. Returns routing recommendations. No side effects.",
        inputSchema: {
            type: "object",
            properties: {
                task_description: {
    ...
    },
    {
        ...
        description: "Collect and synthesize results from a parallel governance review session. Produces a unified APPROVE/ESCALATE/BLOCK decision from individual reviewer assessments. Use after aci_parallel_review to aggregate domain expert opinions. Returns unified decision with confidence and dissent analysis.",
        inputSchema: {
            type: "object",
            properties: {
                review_session_id: {
    ...
    },
    {
        ...
        description: "Explicitly record a governance decision in the append-only audit log. Use when decisions are made outside the classify pipeline (e.g., manual human approvals, out-of-band escalations). Returns decision ID and timestamp. Side effect: writes to immutable audit log.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        ...
        description: "Semantic search over past governance decisions to find similar precedents. Use when making governance decisions to check how similar actions were classified historically. Returns top-K precedents with similarity scores and outcomes. No side effects -- read-only search.",
        inputSchema: {
            type: "object",
            properties: {
                query: {
    ...
    },
    {
        ...
        description: "Record the actual outcome of a previously classified governance decision. Feeds the calibration loop to improve future classification accuracy. Use after an approved action completes (or fails) to close the feedback loop. Side effect: updates the decision record in the audit log.",
        inputSchema: {
            type: "object",
            properties: {
                decision_id: {
    ...
    },
    {
        ...
        description: "Estimate the blast radius of a proposed action -- affected systems, users, data records, cascade depth, and recovery time. Use before approving high-risk actions to understand potential impact. Returns quantitative impact metrics and affected dependency graph. No side effects -- read-only analysis.",
        inputSchema: {
            type: "object",
            properties: {
                action: {
    ...
    },
    {
        ...
        description: "Get aggregate governance health metrics for the organization. Use for governance dashboards, compliance reporting, and calibration monitoring. Returns classification distribution, calibration accuracy, false positive/negative rates, and overall health score. No side effects -- read-only metrics query.",
        inputSchema: {
            type: "object",
            properties: {
                time_range: {
    ...
    },
    {
        ...
        description: "Send a message in an active interview session and receive the next adaptive question. Use to conduct stakeholder interviews step by step. Returns the AI-generated follow-up question and session progress. Local tool -- no API key charges.",
        inputSchema: {
            type: "object",
            properties: {
                token: {
    ...
    },
    {
        ...
        description: "Generate an AI summary from an interview session's transcript. Includes key findings, pain points, time/cost estimates, and prioritized recommendations. Use after completing an interview to produce a structured deliverable. Local tool -- no API key charges. No side effects beyond reading the session.",
        inputSchema: {
            type: "object",
            properties: {
                token: {
    ...
    },
    {
        ...
        description: "List all interview sessions with optional status filter. Use to find active, completed, or expired sessions. Returns session metadata including customer name, stakeholder, and creation time. Local tool -- no API key charges. No side effects -- read-only query.",
        inputSchema: {
            type: "object",
            properties: {
                status: {
 ...
 */
export function createMcpServer(apiClient) {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            switch (name) {
                case "s2t_embed":
                    result = await handleEmbed(args, apiClient);
                    break;
                case "s2t_generate_cloudformation":
                    result = await handleCloudFormation(args, apiClient);
                    break;
                case "s2t_validate_oauth":
                    result = await handleOAuthValidate(args, apiClient);
                    break;
                case "s2t_validate_iam_policy":
                    result = await handleIamPolicyValidate(args, apiClient);
                    break;
                case "s2t_validate_mfa_compliance":
                    result = await handleMfaCompliance(args, apiClient);
                    break;
                case "s2t_generate_dynamodb_design":
                    result = await handleDynamoDbDesign(args, apiClient);
                    break;
                case "s2t_analyze_error_patterns":
                    result = await handleErrorPatterns(args, apiClient);
                    break;
                case "s2t_check_data_lake_readiness":
                    result = await handleDataLakeReadiness(args, apiClient);
                    break;
                case "s2t_catalog":
                ...
                case "s2t_classify_action_risk":
                    result = await handleRiskClassify(args, apiClient);
                    break;
                case "s2t_route_task_to_agent":
                    result = await handleTaskRouting(args, apiClient);
                    break;
                case "s2t_predict_system_issues":
                    result = await handlePredictIssues(args, apiClient);
                    break;
                case "s2t_attempt_auto_recovery":
                    result = await handleAutoRecovery(args, apiClient);
                    break;
                case "s2t_execute_with_resilience":
                    result = await handleResilienceExecute(args, apiClient);
                    break;
                case "s2t_manage_agent_memory":
                    result = await handleAgentMemory(args, apiClient);
                    break;
                case "s2t_submit_agent_task":
                    result = await handleAgentTask(args, apiClient);
                    break;
                case "s2t_create_trace_context":
                    result = await handleTraceContext(args, apiClient);
                    break;
                case "s2t_acquire_file_lock":
                    result = await handleFileLock(args, apiClient);
                    break;
                case "s2t_validate_cli_readiness":
                    result = await handleCliReadiness(args, apiClient);
                    break;
                // ACI Governance Tools
                case "aci_classify_decision":
                    result = await handleAciClassify(args, apiClient);
                    break;
                case "aci_financial_gate":
                    result = await handleAciFinancialGate(args, apiClient);
                    break;
                case "aci_compliance_check":
                    result = await handleAciComplianceCheck(args, apiClient);
                    break;
                case "aci_route_domain":
                    result = await handleAciRouteDomain(args, apiClient);
                    break;
                case "aci_parallel_review":
                    result = await handleAciParallelReview(args, apiClient);
                    break;
                case "aci_synthesize_reviews":
                    result = await handleAciSynthesizeReviews(args, apiClient);
                    break;
                case "aci_log_decision":
                    result = await handleAciLogDecision(args, apiClient);
                    break;
                case "aci_recall_precedent":
                    result = await handleAciRecallPrecedent(args, apiClient);
                    break;
                case "aci_record_outcome":
                    result = await handleAciRecordOutcome(args, apiClient);
                    break;
                case "aci_estimate_blast_radius":
                    result = await handleAciBlastRadius(args, apiClient);
                    break;
                case "aci_generate_rollback":
                    result = await handleAciRollback(args, apiClient);
                    break;
                case "aci_governance_health":
                    result = await handleAciGovernanceHealth(args, apiClient);
                    break;
                // Local tools (powered by @s2t/core)
                case "s2t_interview_create":
                    result = await handleInterviewCreate(args);
                    break;
                case "s2t_interview_message":
                    result = await handleInterviewMessage(args);
                    break;
                case "s2t_interview_summary":
                    result = await handleInterviewSummary(args);
                    break;
                case "s2t_interview_list":
                    result = await handleInterviewList(args);
                    break;
                default:
